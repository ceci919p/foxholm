...
<template>
    ...
    <div id="primary" class="content-area">
        <main id="main" class="site-main">
            ...
            <nav id="filtrering">
				<p>Vælg størrelse:</p>
				<button class="filter" data-sko="alle">Alle</button>
			</nav>
        ...
        const url = "https://ceciliejasmin.dk/kea/10_eksamensprojekt/foxholm/wordpress/wp-json/wp/v2/sko";
        const stoerrelseUrl = "https://ceciliejasmin.dk/kea/10_eksamensprojekt/foxholm/wordpress/wp-json/wp/v2/skostoerrelse/";
        async function getJson() {
            
            const data = await fetch(url);
            const stoerrelsedata = await fetch(stoerrelseUrl);
            skoflertal = await data.json();
            stoerrelser = await stoerrelsedata.json();
            console.log(stoerrelser);
            //kald til funktionen visSkoflertal
            visSkoflertal();
        ...
        function opretknapper () {
            stoerrelser.forEach(stoerrelse =>{
                ...
                document.querySelector("#filtrering").innerHTML += `<button class="filter" data-sko="${stoerrelse.name}">${stoerrelse.name}</button>`
        ...
        function filtrering(){
            //variablen der holder styr på hvilken kategori der er blevet valgt er let filterSko.
            ...
            console.log(filterSko);
            visSkoflertal();
        }

        function visSkoflertal () {
            console.log(skoflertal);
            
            const liste = document.querySelector("#oversigt");
            const skabelon = document.querySelector("template");
            liste.textContent = "";
            skoflertal.forEach(sko => {
                //Hvis arrayet viser tal skal filterSko også skal laves om til tal. Dette gøres med parseInt() - så det ville hedde (parseInt(filterSko)). I mit tilfælde havde jeg tekst og derfor skulle filterSko forblive tekst.
                console.log(sko.stoerrelse);
                if (filterSko == "alle" || sko.stoerrelse.includes(filterSko)){
                const klon = skabelon.cloneNode(true).content;
                klon.querySelector("img").src = sko.billede.guid;
                klon.querySelector(".titel").textContent = sko.title.rendered;
                klon.querySelector(".maerke").textContent = "Mærke: " + sko.maerke;
                klon.querySelector(".stoerrelse").textContent = "Størrelse: " + sko.stoerrelse;
                klon.querySelector(".pris").innerHTML = "Pris: " + sko.pris + " kr";
                klon.querySelector("article").addEventListener("click", () => {
                    location.href = sko.link; })
